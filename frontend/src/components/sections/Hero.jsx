...
const Hero = () => (
  ...
  >
    ...
    <div className="absolute -bottom-20 -left-20 w-[360px] h-[360px] rounded-full pointer-events-none"
         style={{ background: 'radial-gradient(circle,rgba(244,140,6,0.06) 0%,transparent 70%)' }} />
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 w-full
      ...
      <div className="anim-up order-2 lg:order-1">
        ...
        <p className="text-base sm:text-lg text-gray-500 leading-relaxed mb-9 max-w-lg">
          Bridging the gap between grassroots activism and modern technology
          to create sustainable social impact across communities.
        </p>
          ...
          {[
            ...
            <div key={s.l}>
              <p className="text-2xl sm:text-3xl font-black text-gray-900 leading-none"
                 style={{ fontFamily: 'Outfit, sans-serif' }}>{s.v}</p>
      ...
      {/* ── Right: Image ── */}
      <div className="relative anim-up delay-200 order-1 lg:order-2">
        ...
        <div className="absolute bottom-4 left-4 flex items-center gap-3
                        bg-white/95 backdrop-blur-md rounded-2xl px-4 py-3
                        shadow-lg border border-white/60">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center"
               style={{ background: 'linear-gradient(135deg,#E85D04,#F48C06)' }}>
            <i className="ri-group-fill text-white text-base leading-none" />
          </div>
          <div>
            <p className="text-base font-black text-gray-900 leading-none"
               style={{ fontFamily: 'Outfit, sans-serif' }}>48+</p>
            <p className="text-xs text-gray-500 font-medium">Lives Impacted</p>
          </div>
        </div>

        {/* Decorative floats */}
        <div className="absolute -top-4 -right-4 w-16 h-16 rounded-2xl anim-float"
             style={{ background: 'linear-gradient(135deg,rgba(232,93,4,0.12),rgba(244,140,6,0.12))' }} />
        <div className="absolute -bottom-3 right-10 w-10 h-10 rounded-xl anim-float delay-200"
             style={{ background: 'linear-gradient(135deg,rgba(232,93,4,0.10),rgba(244,140,6,0.10))' }} />
      </div>
    </div>
  </section>
)
